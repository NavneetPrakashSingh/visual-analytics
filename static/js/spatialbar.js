d3.csv("static/data/Final_Dataset.csv", function(err, data) {
    ...
    console.log(data)
    var config = {"data0":"Country","data1":"expectancy ",
                "label0":"label 0","label1":"label 1","color0":"#99ccff","color1":"#0050A1",
                "width":760,"height":600}
                
  //  var locations = config.data1;
    var set_years = [];
    var set_country = [];
   ...
   // console.log(min_year,max_year);
    //console.log(years);
    var svg_slider = d3.select("#year_slider")
  ...
  console.log(country_set.has(country_name));
  //d3.select("#bar").exit().remove();
  ...
  var y5 = d3.scaleLinear()
          ...
          if(d.Country == country_name && d.Year == 2006) {
             return d;}
                     })
      console.log(data1);
    ...
    x1.domain(data1.map(function(d) { return d.Country; }));
    y1.domain([0, d3.max(data1, function(d) { return d["expectancy "]; })]);
    y2.domain([0, d3.max(data1, function(d) { return d["infant deaths"]; })]);
    y3.domain([0, d3.max(data1, function(d) { return d["Adult Mortality"]; })]);
    y4.domain([0, d3.max(data1, function(d) { return d[" HIV/AIDS"]; })]);
    y5.domain([0, d3.max(data1, function(d) { return d[" BMI "]; })]);
    //y6.domain([0, d3.max(data1, function(d) { return d.cervical_cancer_number_of_new_female_cases; })]);
  ...
  d3.select("#container5").remove();	  
        //https://stackoverflow.com/questions/33628552/two-bar-charts-from-same-data-object-using-d3-js
...
.style("text-decoration", "bold");
  ...
  container3.attr('transform','translate('+ width/5 +',0)'); //To move the g container to the right.
  container4.attr('transform','translate('+ width/3.2 +',0)'); //To move the g container to the right.
  container5.attr('transform','translate('+ width/2.5 +',0)'); //To move the g container to the right.
